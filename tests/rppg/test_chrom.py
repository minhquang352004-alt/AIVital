...
class TestCHROMMethod(unittest.TestCase):
    """Kiểm thử thuật toán CHROM (Chrominance-based) trong rppg/chrom.py."""
    ...
    def test_chrom_accuracy_and_sqi(self):
        chrom = CHROMMethod(fps=self.fs, lowcut=0.75, highcut=2.50)
        bvp = chrom.process(self.synthetic_rgb)
    ...
    def test_chrom_metadata(self):
        """Streaming API: update() + get_signal() → get_metadata() trả metadata hợp lệ."""
        chrom = CHROMMethod(fps=self.fs)
        chrom.update(self.synthetic_rgb)
        chrom.get_signal()
        meta = chrom.get_metadata()
        self.assertEqual(meta["method"], "CHROM")
